...
where $v^\star_{jk}$ and $u^\star_{ik}$ are the vectors of scores obtained by weighted averaging for CA for axis $k$ (respectively equal to `co` and `li`).
...
where $u^\star_{ik}$ is the coordinate of row (site) $i$ on axis $k$ and $v^\star_{jk}$ is the same for column (species) $j$ and $\mu_k = 1 + \sqrt{\lambda_k}$.
...
cancorRC <- cancor(diag(sqrt(wt)) %*% tabR_scaled, 
                   diag(sqrt(wt)) %*% tabC_scaled, 
                   xcenter = FALSE, ycenter = FALSE) # set to false because we did our own centering before

# cancorRC gives the coefficients of the linear combinations that maximizes the correlation between the 2 dimensions
ncol(cancorRC$xcoef) # r-1 columns -> R_scaled is not of full rank
ncol(cancorRC$ycoef) # c-1 columns -> C_scaled is not of full rank

# Compute these scores from this coef
scoreR <- tabR_scaled[, 1:(r-1)]  %*% cancorRC$xcoef
scoreC <- tabC_scaled[, 1:(c-1)]  %*% cancorRC$ycoef
```

```{r}
# t(cancorRC$xcoef) %*% diag(1, nrow(cancorRC$xcoef)) %*% cancorRC$xcoef

# scoreR and scoreC are normed to wt
res <- t(scoreR) %*% diag(wt) %*% scoreR # R^t Dw R = 1
all(abs(res - diag(1, ncol(scoreR))) < zero) 

res <- t(scoreC) %*% diag(wt) %*% scoreC # C^t Dw C = 1
all(abs(res - diag(1, ncol(scoreC))) < zero)
```

Below, we reproduce the same analysis, but we don't multiply matrices with weights:

```{r}
cancorRC2 <- cancor(tabR_scaled, tabC_scaled, 
                    xcenter = FALSE, ycenter = FALSE)

# Compute these scores from this coef
scoreR2 <- tabR_scaled[, 1:(r-1)]  %*% cancorRC2$xcoef[, 1:(r-1)]
scoreC2 <- tabC_scaled[, 1:(c-1)]  %*% cancorRC2$ycoef[, 1:(c-1)]

# Scores 2 are not normed at wt
res <- t(scoreR2) %*% diag(wt, nrow(scoreR2)) %*% scoreR2 # R2^t Dw R2 =/= 1
all(abs(res - diag(1, ncol(scoreR2))) < zero) 

res <- t(scoreC2) %*% diag(wt, nrow(scoreC2)) %*% scoreC2 # C2^t Dw C2 =/= 1
all(abs(res - diag(1, ncol(scoreC2))) < zero)

# They are normed at 1
res <- t(scoreR2) %*% diag(1, nrow(scoreR2)) %*% scoreR2 # R2^t D1 R2 = 1
all(abs(res - diag(1, ncol(scoreR2))) < zero)

res <- t(scoreC2) %*% diag(1, nrow(scoreC2)) %*% scoreC2 # C2^t D1 C2 = 1
all(abs(res - diag(1, ncol(scoreC2))) < zero)
```
...
#### Canonical correlation analysis "by hand"

We try to compute the canonical correlation "by hand":

```{r}
# Unweighted
matR <- scale(tabR, scale = FALSE, 
              center = FALSE)
matC <- scale(tabC, scale = FALSE, 
              center = FALSE)
...
# svdK$d[1:(c-1)]/cancorRC$cor
# xcoefK[1:(r-1), 1:(c-1)]/cancorRC$xcoef[, 1:(c-1)]
# ycoefK[1:(c-1), 1:(c-1)]/cancorRC$ycoef[, 1:(c-1)]
# 
# scoreRK[, 1:(c-1)]/scoreR[, 1:(c-1)]
# scoreCK[, 1:(c-1)]/scoreC[, 1:(c-1)]

# SRR <- t(tabR_scaled) %*% diag(sqrt(wt)) %*% tabR_scaled
# SRC <- t(tabR_scaled) %*% diag(sqrt(wt)) %*% tabC_scaled
# SCC <- t(tabC_scaled) %*% diag(sqrt(wt)) %*% tabC_scaled
# 
# K <- solve(chol(t(SRR))) %*% SRC %*% solve(chol(SCC)) 
# 
# # SVD
# svdK <- svd(K)
# # Canonical coefficients
# xcoefK <- solve(chol(t(SRR))) %*% svdK$u
# ycoefK <- solve(chol(t(SCC))) %*% svdK$v
# 
# svdK$d[1:(c-1)]/cancorRC$cor
# xcoefK[1:(r-1), 1:(c-1)]/cancorRC$xcoef[, 1:(c-1)]
...
|-------------------|------------------------------|------------------------|
| Mean for axis $k$                | $$m_{ik} = \frac{1}{y_{i\cdot}} \sum_{j = 1}^c y_{ij} h_k(i, j)$$                           | $$m_{jk} = \frac{1}{y_{\cdot j}} \sum_{i = 1}^r y_{ij} h_k(i, j)$$                           |
| Variance for axis $k$            | $$s^2_{ik} = \frac{1}{y_{i\cdot}} \sum_{j = 1}^c y_{ij} \left(h_k(i, j) - m_{ik}\right)^2$$ | $$s^2_{jk} = \frac{1}{y_{\cdot j}} \sum_{i = 1}^r y_{ij} \left(h_k(i, j) - m_{jk}\right)^2$$ |
| Covariance between axes $k$, $l$ | $$c_{kl}(i) = \frac{1}{y_{i \cdot}}\sum_{j=1}^c y_{ij}h_k(i, j)h_l(i, j) - m_{ik}m_{il}$$  | $$c_{kl}(j) = \frac{1}{y_{\cdot j}}\sum_{i=1}^r y_{ij}h_k(i, j)h_l(i, j) - m_{jk}m_{jl}$$   |
...
|                                  | Rows (sites)                                                                                                                                                                                | Columns (species)                                                                                                                                                                           |
|-------------------|------------------------------|------------------------|
| Mean for axis $k$                | $$m_{ik} = \frac{\sqrt{\mu_k}}{\sqrt{2\lambda_k}} u^\star_{ik}$$                                                                                                                                  | $$m_{jk} = \frac{\sqrt{\mu_k}}{\sqrt{2\lambda_k}} v^\star_{jk}$$                                                                                                                                  |
| Variance for axis $k$            | $$s^2_{ik} = \frac{1}{2\lambda_k\mu_k} \left( \frac{1}{y_{i \cdot}} \sum_{j = 1}^c \left(y_{ij} {v^\star_{jk}}^2 \right) - \lambda_k {u^\star_{ik}}^2 \right)$$                                             | $$s^2_{jk} = \frac{1}{2\lambda_k\mu_k} \left( \frac{1}{y_{\cdot j}} \sum_{i = 1}^r \left(y_{ij} {u^\star_{ik}}^2 \right) - \lambda_k {v^\star_{jk}}^2 \right)$$                                             |
| Covariance between axes $k$, $l$ | $$c_{kl}(i) = \frac{1}{2\sqrt{\lambda_k \lambda_l} \sqrt{\mu_k \mu_l}} \left( \frac{1}{y_{i \cdot}} \sum_{j = 1}^c y_{ij} v^\star_{jk} v^\star_{jl} - \sqrt{\lambda_k \lambda_l} u^\star_{ik}u^\star_{il} \right)$$ | $$c_{kl}(j) = \frac{1}{2\sqrt{\lambda_k \lambda_l} \sqrt{\mu_k \mu_l}} \left( \frac{1}{y_{\cdot j}} \sum_{i = 1}^r y_{ij} u^\star_{ik} u^\star_{il} - \sqrt{\lambda_k \lambda_l} v^\star_{jk}v^\star_{jl} \right)$$ |

***Note: formulas for the variances $s^2_{ik}$ and $s^2_{jk}$ are different from formula (15) in @thioulouse1992 (no square root at the denominator  $2\lambda_k\mu_k$).***
:::
...
$$
m_{ik} = \frac{1}{y_{i\cdot}} \sum_{j = 1}^c y_{ij} h_k(i, j)
$$
...
$$
s^2_{ik} = \frac{1}{y_{i\cdot}} \sum_{j = 1}^c y_{ij} \left(h_k(i, j) - m_{ik}\right)^2
$$ 

*NB: in the original article [@thioulouse1992] they use the compact variance formula, $s^2{ik} = \frac{1}{y_{i\cdot}} \sum_{j = 1}^c \left(y_{ij} h_k(i, j)^2\right) - m_{ik}^2$ but I prefer the other formula.*
...
$$
c_{kl}(i) = \frac{1}{y_{i \cdot}}\sum_{j=1}^c y_{ij}h_k(i, j)h_l(i, j) - m_{ik}m_{il}
$$
...
Below, we compute the covariance between axes $k =$ `r kk` and $l =$ `r lk`.
...
covrow1 <- (1/yi_[i])*sum(Yfreq0$Freq[rows] * H[rows, kk] * H[rows, lk]) - mrows[i, kk]*mrows[i, lk]
```
...
df <- data.frame(var = Yfreq0$Freq*H[, kk]*H[, lk], 
                 by = Yfreq0$row)
...
covrows_kl <- (1/yi_)*sum_per_site - mrows[, kk]*mrows[, lk]
```
...
covrows_kl2 <- sapply(covrows, function(c) c[kk, lk])
...
covrows_plot <- s.class(rec_ca[,  kk:lk], 
                        fac = rec_ca$Row,
                        wt = rec_ca$Weight, 
                        plot = FALSE)
covrows_kl_plot <- sapply(covrows_plot@stats$covvar, function(x) x[kk, lk])
...
$$
m_{jk} = \frac{1}{y_{\cdot j}} \sum_{i = 1}^r y_{ij} h_k(i, j)
$$
...
$$
s^2_{jk} = \frac{1}{y_{\cdot j}} \sum_{i = 1}^r y_{ij} \left(h_k(i, j) - m_{jk}\right)^2
$$
...
$$c_{kl}(j) = \frac{1}{y_{\cdot j}}\sum_{i=1}^r y_{ij}h_k(i, j)h_l(i, j) - m_{jk}m_{jl}$$
...
where $u^\star_{ik}$ corresponds to the `li` for site $i$ on axis $k$.
...
$$
s^2_{ik} = \frac{1}{2\lambda_k\mu_k} \left( \frac{1}{y_{i \cdot}} \sum_{j = 1}^c \left(y_{ij} {v^\star_{jk}}^2 \right) - \lambda_k {u^\star_{ik}}^2 \right)
$$ 
...
$$
c_{kl}(i) = \frac{1}{2\sqrt{\lambda_k \lambda_l} \sqrt{\mu_k \mu_l}} \left( \frac{1}{y_{i \cdot}} \sum_{j = 1}^c y_{ij} v^\star_{jk} v^\star_{jl} - \sqrt{\lambda_k \lambda_l} u^\star_{ik} u^\star_{il} \right)
$$
...
$$
s^2_{jk} = \frac{1}{2\lambda_k\mu_k} \left( \frac{1}{y_{\cdot j}} \sum_{i = 1}^r \left(y_{ij} {u^\star_{ik}}^2 \right) - \lambda_k {v^\star_{jk}}^2 \right)
$$
...
$$
c_{kl}(j) = \frac{1}{2\sqrt{\lambda_k \lambda_l} \sqrt{\mu_k \mu_l}} \left( \frac{1}{y_{\cdot j}} \sum_{i = 1}^r y_{ij} u^\star_{ik} u^\star_{il} - \sqrt{\lambda_k \lambda_l} v^\star_{jk}v^\star_{jl} \right)
$$
...
We use the notation $m_{ik}$/$s^2_{ik}$ for the mean/variances of sites (index $i$) computed with the RC scores and $mR_{ik})$/$sR^2_{ik}$ for the mean/variances of sites computed with the R score. For the means computed for species, we use index $j$ and for the means/variances computed with C scores, we use $mC$/$sC^2$.
...
|                       | Rows (sites)                      | Columns (species)        |
|-----------------------|-----------------------------------|--------------------------|
| Mean for axis $k$     | $$mR_{ik} = \frac{1}{y_{i\cdot}} \sum_j y_{ij} ~ \text{scoreR}_k(i, j)$$ | $$mR_{jk} = \frac{1}{y_{\cdot j}} \sum_i y_{ij} ~ \text{scoreR}_k(i, j)$$ |                                      
| Variance for axis $k$ |  $$sR^2_{ik} = \frac{1}{y_{\cdot j}} \sum_j y_{ij} ~ \text{scoreR}_k(i, j)^2 - mR_{ik}^2$$ | $$sR^2_{jk} = \frac{1}{y_{i \cdot}} \sum_j y_{ij} ~ \text{scoreR}_k(i, j)^2 - mR_{jk}^2$$ |
...
|                       | Rows (sites)                      | Columns (species)        |
|-----------------------|-----------------------------------|--------------------------|
| Mean for axis $k$     | $$mC_{ik} = \frac{1}{y_{i\cdot}} \sum_j y_{ij} ~ \text{scoreC}_k(i, j)$$ | $$mC_{jk} = \frac{1}{y_{\cdot j}} \sum_i y_{ij} ~ \text{scoreC}_k(i, j)$$ |
| Variance for axis $k$ | $$sC^2_{ik} = \frac{1}{y_{\cdot j}} \sum_j y_{ij} ~ \text{scoreC}(i, j)^2 - mC_{ik}^2$$ | $$sC^2_{jk} = \frac{1}{y_{i \cdot}} \sum_j y_{ij} ~ \text{scoreC}(i, j)^2 - mC_{jk}^2$$ |
| Covariance between axes $k$, $l$ | | |
...
|                       | Rows (sites)                        | Columns (species)        |
|-----------------------|-------------------------------------|--------------------------|
| Mean for axis $k$     | $\frac{1}{\sqrt{n}} u_{ik}$ | $\frac{1}{\sqrt{n}} v^\star_{jk}$ |
| Variance for axis $k$ | None                                | $$\frac{1}{n \lambda_k} \left( \frac{1}{y_{\cdot j}} \sum_{i = 1}^r \left(y_{ij} {u^\star_{ik}}^2 \right) - \lambda_k {v^\star_{ik}}^2 \right)$$ |
...
|                       | Rows (sites)                      | Columns (species)        |
|-----------------------|-----------------------------------|--------------------------|
| Mean for axis $k$     | $u^\star_{ik} \times \frac{1}{\sqrt{n}}$ | $v_{jk} \times \frac{1}{\sqrt{n}}$ |
| Variance for axis $k$ |  $$\frac{1}{n \lambda_k} \left( \frac{1}{y_{i \cdot}} \sum_{j = 1}^c \left(y_{ij} {v^\star_{jk}}^2 \right) - \lambda_k {u^\star_{ik}}^2 \right)$$ | None                   |
| Covariance between axes $k$, $l$ |                         | None                    |
...
|                       | Rows (sites)                      | Columns (species)        |
|-----------------------|-----------------------------------|--------------------------|
| Mean for axis $k$     | $$mR_{ik} = \frac{\sqrt{2}}{\sqrt{\mu_k n}} m_{ik}$$ | $$mR_{jk} = \frac{\sqrt{2 \lambda_k}}{\sqrt{\mu_k n}} m_{jk}$$ |
| Variance for axis $k$ | None | $$sR^2_{jk} = \frac{2 \mu_k}{n} s^2_{jk}$$ |
| Covariance between axes $k$, $l$ |  | |
...
|                       | Rows (sites)                      | Columns (species)        |
|-----------------------|-----------------------------------|--------------------------|
| Mean for axis $k$     | $$mC_{ik} =  \frac{\sqrt{2 \lambda_k}}{\sqrt{\mu_k n}} m_{ik}$$ | $$mC_{jk} = \frac{\sqrt{2}}{\sqrt{\mu_k n}} m_{jk} $$ |
| Variance for axis $k$ | $$sC^2_{ik} = \frac{2 \mu_k}{n} s^2_{ik}$$ | None |
| Covariance between axes $k$, $l$ |  | |

:::

This is not at all developed in the article of @thioulouse1992, but we could also use directly the R scores and the C scores to compute the groupwise (species- or sites-wise) mean and variances. In fact, the means and variances computed from the scoresR and scoresC should be related to the means and variances computed from $u^\star$ = `li` and $v^\star$ = `co`, since scoreR = `l1` and scoreC = `c1`.
...
We have $mC_{ik} = \frac{u^\star_{ik}}{\sqrt{n}}$.
...
$$mC_{ik} = \frac{1}{y_{i\cdot}} \sum_i y_{ij} ~ \text{scoreC}_k(i, j)$$
We replace scoreC with $\frac{v_{jk}}{\sqrt{n}}$ (this equality remains to be proven):

$$mC_k(i) = \frac{1}{y_{i\cdot}} \sum_i y_{ij} \frac{v_{jk}}{\sqrt{n}}$$

Then we remove $\frac{1}{n}$ from the sum:

$$mC_{ik} = \frac{1}{\sqrt{n}}\frac{1}{y_{i\cdot}} \sum_i y_{ij} v_{jk}$$
By definition, the `li` are at the mean of the `c1` so we can replace $\frac{1}{y_{i\cdot}} \sum_i y_{ij} v_{jk}$ with $u^\star_{jk}$:

$$mC_{ik}  = \frac{1}{\sqrt{n}} u^\star_{jk}$$
...
We have $sC_{ik}^2 = \frac{2\mu_k}{n} ~ s^2_{ik}$.
...
\begin{align}
sC_{ik}^2 
&= \frac{1}{y_{\cdot j}} \sum_j y_{ij} ~ \text{scoreC}(i, j)^2 - mC_{ik}^2\\ 
&= \frac{1}{y_{\cdot j}} \sum_j y_{ij} ~ \left(\frac{v_{jk}}{\sqrt{n}}\right)^2 - \left(\frac{u^\star_{ik}}{\sqrt{n}}\right)^2\\ 
&= \frac{1}{n} \left(\frac{1}{y_{\cdot j}} \sum_j y_{ij} ~ v_{jk}^2 - {u^\star_{ik}}^2\right)\\ 
&= \frac{1}{n} \left(\frac{1}{y_{\cdot j}} \sum_j y_{ij} ~ \left( \frac{v^\star_{jk}}{\sqrt{\lambda_k}}\right)^2 - {u^\star_{ik}}^2\right)\\ 
&= \frac{1}{n} \times \frac{1}{\lambda_k} \left(\frac{1}{y_{\cdot j}} \sum_j y_{ij} ~ {v^\star_{jk}}^2 - \lambda_k {u^\star_{ik}}^2\right)\\ 
&= \frac{1}{n} \times \frac{1}{\lambda_k} \times 2\lambda_k \mu_k \times \frac{1}{2\lambda_k \mu_k} \left(\frac{1}{y_{\cdot j}} \sum_j y_{ij} ~ {v^\star_{jk}}^2 - \lambda_k {u^\star_{ik}}^2\right)\\ 
...
We have $mR_{ik} = \frac{u_{ik}}{\sqrt{n}}$.
...
We have $mC_{jk} = \frac{v_{jk}}{\sqrt{n}}$.
...
We have $mR_{jk} = \frac{v^\star_{jk}}{\sqrt{n}}$.
...
We have $sR^2_{jk} = \frac{2\mu_k}{n} ~ s^2_{jk}$.
...
$$u^\star_{ik} = \sum_{j=1}^c p_{j|i} v^\star_{jk}$$
```{r}
...
$$v^\star_{ik} = \frac{1}{\sqrt{\lambda_k}}\sum_{j=1}^c p_{j|i} u^\star_{jk}$$
...
s.class(H1, fac = Yfreq0$row, 
        wt = Yfreq0$Freq,
        plabels.col = params$colsite,
        main = "Non-reciprocal scaling")
s.class(H, fac = Yfreq0$row, 
        wt = Yfreq0$Freq,
        plabels.col = params$colsite,
...
```{r}
#| code-fold: true
colfac <- factor(Yfreq0$col)
s.class(H2, fac = colfac, 
        wt = Yfreq0$Freq,
        plabels.col = params$colspp,
        main = "Non-reciprocal scaling")
s.class(H, fac = colfac, 
        wt = Yfreq0$Freq,
        plabels.col = params$colspp,
        main = "Reciprocal scaling")
```
