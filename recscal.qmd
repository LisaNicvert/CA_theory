...
```{r}
k <- min(r - 1, c - 1) # non-null eigenvalues
...
If we don't wand to find a new common space, and instead remain in the 2 biplots showing $U$ and $A$ (`co` and `l1`) and $V$ with $B$ (`c1` and `li`), we can.
...
s.label(ca$li[i, ], 
        plabels.col = params$colsite, add = TRUE)
```
If we were to compute these CA correspondences in the other biplot, it would be:
$$\tilde{h}_k(i, j) = \frac{u_k(i) + a_k(j)}{2}$$
...
The variances are equivalent to a "weighted variance" (variance developed formula: $\frac{\sum x_i^2}{n} - \bar{x}^2$).

|                                  | Rows (sites)                                                                                                                                                                                | Columns (species)                                                                                                                                                                           |
|-------------------|------------------------------|------------------------|
| Mean for axis $k$                | $$m_k(i) = \frac{\sqrt{\mu_k}}{\sqrt{2\lambda_k}} b_k(i)$$                                                                                                                                  | $$m_k(j) = \frac{\sqrt{\mu_k}}{\sqrt{2\lambda_k}} a_k(j)$$                                                                                                                                  |
...
| Covariance between axes $k$, $l$ | $$c_{kl}(i) = \frac{1}{2\sqrt{\lambda_k \lambda_l} \sqrt{\mu_k \mu_l}} \left( \frac{1}{y_{i \cdot}} \sum_{j = 1}^c y_{ij} a_k(j) a_l(j) - \sqrt{\lambda_k \lambda_l} b_k(i)b_l(i) \right)$$ | $$c_{kl}(j) = \frac{1}{2\sqrt{\lambda_k \lambda_l} \sqrt{\mu_k \mu_l}} \left( \frac{1}{y_{\cdot j}} \sum_{i = 1}^r y_{ij} b_k(i) b_l(i) - \sqrt{\lambda_k \lambda_l} a_k(j)a_l(j) \right)$$ |
...
all(abs(recscal_mean/mrows) - 1 < zero)
```
...
all(covrow_ade4[i]/covrow1 - 1 < zero)

# Check with vector method
all(covrow_ade4/covrows - 1 < zero)
```
...
```{r}
#| code-fold: true
# Check agreement with ade4 methods
...
all(abs(recscal_mean/mcols) - 1 < zero)
...
Check that the CA formulas give the same result as the weighted means/variances of correspondences scores:

```{r}
all(mrows2/mrows - 1 < zero)
all(varrows2/varrows - 1 < zero)
```
...
covrows2 <- 1/(2*sqrt(lambda[k]*lambda[l])*sqrt(mu[k]*mu[l]))*(1/yi_*sum_per_site - sqrt(lambda[k]*lambda[l])*B[, k]*B[, l])
```
...
covcols2 <- 1/(2*sqrt(lambda[k]*lambda[l])*sqrt(mu[k]*mu[l]))*(1/y_j*sum_per_spp - sqrt(lambda[k]*lambda[l])*A[, k]*A[, l])
```

Check that the result is the same as with CA formulas:

```{r}
#| code-fold: true
all(mcols2/mcols - 1 < zero)
all(varcols2[-c(7, 13), ]/varcols[-c(7, 13), ] - 1 < zero)

# 2 inf column but it is because variances are zero
all(varcols[c(7, 13), ] < zero)
all(varcols2[c(7, 13), ] < zero)

all(covcols2[-c(7, 13)]/covcols[-c(7, 13)] - 1 < zero)
```
