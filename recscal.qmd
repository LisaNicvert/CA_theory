...
If we don't wand to find a new common space to plot correspondences in, we can remain in the 2 biplots showing $U$ and $V^\star$ (`co` and `l1`) and $V$ with $U^\star$ (`c1` and `li`).

To do that, we define the "CA-correspondences" $\tilde{h}_k(i, j)$:

$$\tilde{h}_k(i, j) = \frac{u^\star_k(i) + v_k(j)}{2}$$
```{r}
...
Below is a graphical demonstration showing the ellipses computed with $\tilde{h}_k(i, j)$. Its mean is surimposed with `li` ($U^\star$) (or it should be, theoretically) and it is similar but smaller than the ellipse computed with `c1`  ($V$):
...
$$\tilde{h}_k(i, j) = \frac{u_k(i) + v^\star_k(j)}{2}$$
