...
*Note: in this part, we used to change the notation and use $L_k(i)$ for `li` ($u^\star$) and $L1_k(i)$ for `l1` ($u$) (and the same with $C_k(j)$ for `co` ($v^\star$) and $C1_k(j)$ for `c1` ($v$)). Some old notation may remain.*
...
&= \frac{1}{y_{\cdot j}} \sum_j y_{ij} ~ \left(\frac{v_{jk}}{\sqrt{n}}\right)^2 - \left(\frac{v^\star_{ik}}{\sqrt{n}}\right)^2\\ 
&= \frac{1}{n} \left(\frac{1}{y_{\cdot j}} \sum_j y_{ij} ~ v_{jk}^2 - {v^\star_{ik}}^2\right)\\ 
